...
module.exports.createsol = async (req, res) => {
  ...
  try {
    if (!subCategory || !automatedWorkflow)
      return res.json({ mssg: "All fields need to be filled" });
    const check = await automatedModel.findOne({ subCategory });
    if (check)
      return res.json({ mssg: "An automated solution already exists" });
    const agent = await agentModel.findOne({ user: req.users._id });
    await automatedModel.create({
      agent: agent._id,
      subCategory,
      automatedWorkflow,
    });
...
module.exports.updatetickets = async (req, res) => {
  ...
  try {
    ...
    await ticketModel.updateOne(
      { _id: ticketid },
      { resolution, status: "closed", closedAt }
    );

    await agentModel.updateOne(
      { user: req.user._id },
      { $pull: { tickets_queue: ticketid }, $inc: { availability: -1 } }
    );

    //Get the tickets in ticket_Manger
    const ticket_Manager = await ticketmanagerModel.findOne({
      Manager_number: 1,
    });
    ...
    for (let i = 0; i < ticket_Manager.high_priority.length; i++) {
      const ticket_details = await ticketModel.findOne({
        _id: ticket_Manager.high_priority[i],
      });
      ...
      if (ticket_details.issueType === "software") {
        ...
        if (softwareAgent.availability < 5) {
          await agentModel.updateOne(
            { _id: softwareAgent._id },
            {
              $push: { tickets_queue: ticket_Manager.high_priority[i] },
              $inc: { availability: 1 },
            }
          );
          await ticketModel.updateOne(
            { _id: ticket_details._id },
            { agent: softwareAgent._id, status: "pending" }
          );
          await ticketmanagerModel.updateOne(
            { Manager_number: 1 },
            { $pull: { high_priority: ticket_Manager.high_priority[i] } }
          );
        }
      }
      if (ticket_details.issueType === "hardware") {
        ...
        if (hardwareAgent.availability < 5) {
          ...
          await ticketModel.updateOne(
            { _id: ticket_details._id },
            { agent: hardwareAgent._id, status: "pending" }
          );
          await ticketmanagerModel.updateOne(
            { Manager_number: 1 },
            { $pull: { high_priority: ticket_Manager.high_priority[i] } }
      ...
      if (ticket_details.issueType === "network") {
        ...
        if (networkAgent.availability < 5) {
          ...
          await ticketModel.updateOne(
            { _id: ticket_details._id },
            { agent: networkAgent._id, status: "pending" }
          );
          await ticketmanagerModel.updateOne(
            { Manager_number: 1 },
            { $pull: { high_priority: ticket_Manager.high_priority[i] } }
    ...
    for (let i = 0; i < ticket_Manager.medium_priority.length; i++) {
      const ticket_details = await ticketModel.findOne({
        _id: ticket_Manager.medium_priority[i],
      });
      ...
      if (ticket_details.issueType === "software") {
        ...
        if (softwareAgent.availability < 5) {
          await agentModel.updateOne(
            { _id: softwareAgent._id },
            {
              $push: { tickets_queue: ticket_Manager.medium_priority[i] },
              $inc: { availability: 1 },
            }
          );
          await ticketModel.updateOne(
          ...
          await ticketmanagerModel.updateOne(
            { Manager_number: 1 },
            { $pull: { medium_priority: ticket_Manager.medium_priority[i] } }
          );
        } else if (softwarehelper.availability < 5) {
          await agentModel.updateOne(
            { _id: softwarehelper._id },
            {
              $push: { tickets_queue: ticket_Manager.medium_priority[i] },
              $inc: { availability: 1 },
            }
          ...
          await ticketModel.updateOne(
            { _id: ticket_details._id },
            { agent: softwarehelper._id, status: "pending" }
          );
          await ticketmanagerModel.updateOne(
            { Manager_number: 1 },
            { $pull: { medium_priority: ticket_Manager.medium_priority[i] } }
      ...
      if (ticket_details.issueType === "hardware") {
        ...
        if (hardwareAgent.availability < 5) {
          ...
          await ticketModel.updateOne(
            { _id: ticket_details._id },
            { agent: hardwarehelper._id, status: "pending" }
          );
          await ticketmanagerModel.updateOne(
            { Manager_number: 1 },
            { $pull: { medium_priority: ticket_Manager.medium_priority[i] } }
      ...
      if (ticket_details.issueType === "network") {
        ...
        if (networkAgent.availability < 5) {
          ...
          await ticketModel.updateOne(
            { _id: ticket_details._id },
            { agent: networkhelper._id, status: "pending" }
          );
          await ticketmanagerModel.updateOne(
            { Manager_number: 1 },
            { $pull: { medium_priority: ticket_Manager.medium_priority[i] } }
    ...
    for (let i = 0; i < ticket_Manager.low_priority.length; i++) {
      const ticket_details = await ticketModel.findOne({
        _id: ticket_Manager.low_priority[i],
      });
      ...
      if (ticket_details.issueType === "software") {
        ...
        if (softwareAgent.availability < 5) {
          await agentModel.updateOne(
            { _id: softwareAgent._id },
            {
              $push: { tickets_queue: ticket_Manager.low_priority[i] },
              $inc: { availability: 1 },
            }
          );
          await ticketModel.updateOne(
          ...
          await ticketmanagerModel.updateOne(
            { Manager_number: 1 },
            { $pull: { low_priority: ticket_Manager.low_priority[i] } }
          );
          ...
          await ticketModel.updateOne(
            { _id: ticket_details._id },
            { agent: shelper._id, status: "pending" }
          );
          await ticketmanagerModel.updateOne(
            { Manager_number: 1 },
            { $pull: { low_priority: ticket_Manager.low_priority[i] } }
      ...
      if (ticket_details.issueType === "hardware") {
        ...
        if (hardwareAgent.availability < 5) {
          ...
          await ticketModel.updateOne(
            { _id: ticket_details._id },
            { agent: hhelper._id, status: "pending" }
          );
          await ticketmanagerModel.updateOne(
            { Manager_number: 1 },
            { $pull: { low_priority: ticket_Manager.low_priority[i] } }
      ...
      if (ticket_details.issueType === "network") {
        ...
        if (networkAgent.availability < 5) {
          ...
          await ticketModel.updateOne(
            { _id: ticket_details._id },
            { agent: nhelper._id, status: "pending" }
          );
          await ticketmanagerModel.updateOne(
            { Manager_number: 1 },
            { $pull: { low_priority: ticket_Manager.low_priority[i] } }
