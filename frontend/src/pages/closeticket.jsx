...
const CloseTicket = () => {
  ...
  const fetchdata = async () => {
    try {
      const response = await axios.get(`http://localhost:5000/api/v1/agentticket/${tid}`, { withCredentials: true });
  ...
  const handlesubmit = async (e) => {
    ...
    try {
      if (resolution === "") {
        ...
        // Ticket is closed
        setShowButton(true);
        const response = await axios.put(`http://localhost:5000/api/v1/updateticket/${tid}`, { resolution: resolution }, { withCredentials: true });
        if (response.status !== 200) {
          navigate('/');
